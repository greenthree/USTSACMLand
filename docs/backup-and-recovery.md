...
永久注销采用外部恢复下限。`delete-account` 的数据库全局租约覆盖“取得租约 → 使用仅有目标仓库 Variables write 的 GitHub fine-grained PAT 更新并回读确认 `BACKUP_RECOVERY_NOT_BEFORE` → 续期并重新确认租约所有权 → 删除 Supabase Auth 用户，等待期间每分钟心跳续期 → 释放租约”的完整临界区；变量只含 UTC 时间，不含成员身份。租约冲突、取得或删除前续期失败、写入或回读确认失败时不得删除 Auth 用户，并返回 `503`；恢复下限已确认但 Auth 删除失败时返回 `409`，保留账号及业务数据；删除期间的心跳故障会触发脱敏告警，但不能掩盖随后确认的成功结果，未释放租约由数据库自动过期。恢复时间再额外增加一小时安全余量，避免运行环境时钟偏差或短暂心跳故障让下限落到实际删除时间之前。

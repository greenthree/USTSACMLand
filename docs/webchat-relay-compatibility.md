...
禁止把三项真实值写入 `.env.example`、命令参数、PR、Issue、截图或聊天。GitHub 工作流只允许 `workflow_dispatch` 手动触发，不监听 `push`、`pull_request`、`schedule` 或 `workflow_run`，避免无意产生费用。默认执行 Abort 和 Prompt Caching 检查；只有排查供应商故障时才可在手动表单中临时关闭，关闭后的结果不能作为完整上线证据。缓存探针会连续发送两次相同的 1024 Token 以上长前缀，因此会产生两次少量模型调用费用。

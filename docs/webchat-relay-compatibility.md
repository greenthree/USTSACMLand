...
真实成员会话对 GPT-5.6 及以后模型使用 typed `input_text` 内容块，在每个用户消息后保留 `prompt_cache_breakpoint: { "mode": "explicit" }`，但不再把请求强制为纯 `explicit` 模式。默认隐式模式会同时使用最新消息自动断点和既有显式断点；下一轮追加消息时，早期用户消息断点保持完全一致，可作为只读缓存命中候选。单次请求最多 40 条消息，因此不会超过服务端最近 50 个断点的读取范围。旧模型和无法明确识别的自定义中转模型不发送显式断点字段，继续使用原有隐式缓存，避免供应商因未知字段返回 `400`。

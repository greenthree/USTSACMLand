...
2026-07-19 的两次生产兼容性试验分别验证了“typed `input_text` + `prompt_cache_breakpoint` + 默认模式”和同一 typed 形状加 `prompt_cache_options.mode=explicit`，当前中转站均在首个请求返回 HTTP 400。失败发生在追加轮之前，因此不能把问题归因于第二轮历史结构；当前可确认的共同不兼容边界是 typed 显式断点形状。真实成员会话和探针因此统一恢复为已验证可成功请求的普通 `role/content` 消息，不发送 `prompt_cache_breakpoint` 或 `prompt_cache_options`，继续携带稳定 `prompt_cache_key`。追加下一轮消息时，系统指令和早期消息对象保持字节不变，由默认隐式缓存尝试复用该精确前缀。

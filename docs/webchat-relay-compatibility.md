...
2026-07-19 的两次生产兼容性试验分别验证了“typed `input_text` + `prompt_cache_breakpoint` + 默认模式”和同一 typed 形状加 `prompt_cache_options.mode=explicit`，当前中转站均在首个请求返回 HTTP 400。失败发生在追加轮之前，因此不能把问题归因于第二轮历史结构；当前可确认的共同不兼容边界是 typed 内容块级显式断点形状。真实成员会话和探针因此使用普通 `role/content` 消息且不发送 `prompt_cache_breakpoint`。独立探针缓存键在流式请求中仅依赖默认策略时两轮缓存均为 0，显式声明请求级 `mode=implicit`、`ttl=30m` 后追加轮命中 1,792 Token；但既有生产缓存键所在的粘性渠道对同一策略返回 HTTP 错误。真实成员请求因此暂不启用该选项，先用探针对生产缓存键和渠道一致性继续隔离。追加下一轮消息时，系统指令和早期消息对象保持字节不变。

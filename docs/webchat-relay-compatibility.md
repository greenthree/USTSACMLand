...
真实成员会话对 GPT-5.6 及以后模型使用 typed `input_text` 内容块，在每个用户消息后保留 `prompt_cache_breakpoint: { "mode": "explicit" }`，并设置请求级 `prompt_cache_options.mode=explicit`。2026-07-19 的生产兼容性试验表明，当前中转站会对“显式断点 + 省略模式”的默认隐式形状返回 HTTP 400，因此本站恢复已知可用的纯显式模式；下一轮追加消息时，早期用户消息断点保持完全一致，可作为只读缓存命中候选。单次请求最多 40 条消息，因此不会超过服务端最近 50 个断点的读取范围。旧模型和无法明确识别的自定义中转模型不发送这两个字段，继续使用原有隐式缓存，避免供应商因未知字段返回 `400`。
...
2. Prompt Caching 探针先发送带长首轮用户消息的 typed Responses 请求，再发送保留同一首轮并追加助手/用户消息的第二轮请求；首个请求的 `input_tokens >= 1024`，第二个请求的 `input_tokens_details.cached_tokens > 0`，否则任务以 `cache_probe_*` 错误失败。该形状同时验证默认隐式写入、历史显式断点读取和真实追加式会话，而不再只证明两个字节完全相同请求能够命中。

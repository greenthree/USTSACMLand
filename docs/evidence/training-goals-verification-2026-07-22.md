...
本记录仍不证明真实生产目标已经通过成功同步达到阈值并完成“确认完成”流程。2026-07-26 已在本地 Supabase 重新执行训练目标 pgTAP，数据库侧的成功快照达标与显式完成流程通过；ROADMAP 继续把本地数据库证明与尚缺的生产自然达标 UI 证据分开记录。
...
### 2026-07-26 本地数据库权限与完成流程复跑

在可用的本地 Supabase 环境执行：

```text
npx --yes supabase@2.109.1 test db supabase/tests/38_training_goals.test.sql --local
```

结果为 `Files=1, Tests=30`、`All tests successful`。这 30 项 pgTAP 真实覆盖：

- RLS、匿名访问拒绝、停用成员拒绝，以及管理员不得越权读写成员目标；
- 成功同步快照推进目标，失败同步不能推进或回退，题数回退显式标记，平台数据缺失保持安全边界；
- 乐观锁冲突、成功快照达到 100%、`complete_own_training_goal` 显式完成，以及重复完成拒绝；
- 目标归档、个人数据导出和账号注销级联。

因此，本地数据库层面的“成功同步快照达到阈值 → 成员显式完成”流程已经得到本轮执行证据；它不替代生产成员通过自然训练进度达到阈值后的 UI 验收。
...
- 尚未在真实生产移动设备上验收训练目标页面；人工屏幕阅读器验收不纳入项目发布范围。
...
同日再次只读核对生产页面：当前账号有 0 个进行中目标、2 个历史目标；两者均为已归档、进度 0%，最近成功同步时间为 2026-07-26 08:01。该状态不能自然进入“已达成待确认”，因此没有创建新目标、触发同步或修改任何生产数据来强行关项。

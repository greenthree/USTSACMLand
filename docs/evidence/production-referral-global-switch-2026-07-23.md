...
使用真实生产管理员登录 `https://ustsacm.fun/admin`，确认后台读取到推荐计划全线关闭、配置版本、最后修改时间、修改人和原因。打开“开启推荐计划”后，正式页面显示二次确认对话框，并强制要求：

- 填写可审计的变更原因；
- 勾选“我已核对全站影响”；
- 两项未完成前“确认开启”保持禁用。

随后使用“取消”关闭对话框，未提交开启操作，生产配置保持关闭。该证据证明正式后台入口和开启方向的二次确认已发布；它不替代关闭方向的正式页面复验、完整焦点顺序、390px 移动端、受控生产双连接围栏或真实邮箱确认注册烟测。

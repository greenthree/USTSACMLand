...
| Supabase Auth / Edge Functions 日志 | Free 计划可查询最近 1 天                                  | 2026-08-13 / Agent 只读核验       | Logs Explorer 可在窗口内查询/导出；超出窗口不可从控制台恢复，Free 计划不含 Log Drains                                       |
| GitHub Actions 日志与 artifact      | 仓库默认 30 天；完整加密快照单项 14 天                    | 2026-08-09 / GitHub API           | 数据库和引用的 WebChat 图片共同进入每日完整快照；约占 `14 × 单次密文大小`                                                   |
| Firecrawl 作业与会话记录            | 浏览器会话最长 1 小时；作业记录固定期限未公开（发布阻塞） | 2026-08-13 / Agent CLI 与官方文档 | 每个 QOJ attempt 主动关闭会话；当前普通请求非 ZDR，供应商隐私政策仅承诺按书面请求删除 PII，未承诺作业记录自动删除或恢复期限 |

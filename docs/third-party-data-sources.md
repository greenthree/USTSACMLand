...
| [clist.by](https://clist.by/)                                     | 服务端请求官方 API 的时间窗口筛选参数；认证凭据仅从 Supabase Function Secrets 读取 | 白名单平台的比赛名称、开始/结束时间、平台与官方比赛链接    | 仅服务端调用 `/api/v4/contest/`；不发送成员资料、不保存原始响应或凭据；API/上游失败时不展示虚构赛事                |

## 保存口径

线上比赛日历现使用 clist.by 官方 `/api/v4/contest/` 接口。API 用户名与 Key 仅从 Supabase Function Secrets 读取，服务端按时间窗口筛选并丢弃白名单之外的赛事；不向上游发送成员资料，也不保存原始响应或凭据。
...
Supabase 备份与日志、GitHub Actions 日志和 Firecrawl 作业记录可能按各自生产配置或服务政策继续保留。本站不能改写已经生成的供应商备份；项目已选择“禁止恢复到注销前”的策略，并在每次受控注销前更新仓库恢复下限。当前正式发布仍受生产保留窗口核验和 Schema v2 隔离恢复演练阻塞；完成前不得正式发布，任何恢复都必须拒绝早于当前恢复下限的备份。

...
> 当前状态：集训队官网首页、生产 Supabase、首管理员、四个 Edge Function 和 35 个 migration 均已部署，前端已连接真实认证与管理接口并由 GitHub Pages 发布。PostgreSQL 17 空库 CI 已通过 16 个 pgTAP 文件、289 项断言；XCPC 共享缓存、Codeforces、洛谷和 QOJ 均有生产烟测证据。自助注销已部署目标绑定租约、事务内 fence 和原子 Auth/Profile 删除边界，但生产仍缺 GitHub 恢复下限写入令牌，因此成功注销继续失败关闭。同步告警 Webhook、首次加密数据库备份与隔离恢复演练也尚未完成。
...
- `/learning`：交互式新手学习引导，包含起点推荐、可持久化四周进度、周任务、可展开五阶段路线、知识地图、训练节奏，以及一键环境配置、算法竞赛 Wiki 和 XCPC Link 等资源入口。

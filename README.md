...
- `/account`：当前用户资料、平台绑定和本人同步；XCPC ELO 显示姓名自动匹配状态，不提供 ID 输入框。
...
后台路由已经做前端管理员守卫，数据库仍以 RLS、最小表权限和管理员 RPC 作为真正安全边界。成员与平台绑定审核使用乐观锁防止基于旧页面误审；审核通过后的首次同步独立执行，同步失败不会回滚审核。统计、快照和同步表仅允许管理员读取，由 service role 写入。手动与审核触发的同步均通过 Edge Function 鉴权，并写入脱敏审计记录。

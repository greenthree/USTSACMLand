...
> 当前状态：集训队官网首页、生产 Supabase、首管理员、四个 Edge Function 和 35 个 migration 均已部署，前端已连接真实认证与管理接口并由 GitHub Pages 发布。PostgreSQL 17 空库 CI 已通过 16 个 pgTAP 文件、289 项断言；XCPC 共享缓存、Codeforces、洛谷、QOJ 和计划同步分页均有生产烟测证据。2026-07-16 已完成首次生产加密逻辑备份、GitHub 端解密自检与下载后的本地独立校验，Artifact 保留 14 天。自助注销仍缺 GitHub 恢复下限写入令牌，成功注销继续失败关闭；同步告警 Webhook与隔离恢复演练也尚未完成。

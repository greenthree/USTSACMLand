    ...
    <div className="page legal-page">
      ...
      <article className="legal-body">
        ...
        <section>
          ...
          <p>
            为支持刷新恢复和历史会话，本站会在 Supabase
            私有数据库中保存会话标题、成员问题和模型可见回复。每个账号最多保留 100
            个会话，单个会话最多 120 条消息且序列化内容不超过 1 MiB；会话最后活动超过 180
            天后自动删除，成员也可以随时删除单个会话。历史读取、修改和删除接口只绑定当前登录账号，不接受目标成员
            ID；管理员默认也不能读取其他成员的对话正文。永久注销账号时，全部会话和消息随账号级联删除。
          </p>
          <p>
            为执行限额、并发控制和故障诊断，另一套私有账本会保存成员
            UUID、随机请求标识、不可逆内容指纹、请求状态、时间及聚合 Token
            数。额度账本本身不保存可还原的对话正文。普通成员只能读取自己的累计请求数、已结算与预留
            Token、累计总限额和剩余额度；成员额度不会每日重置。
          </p>
          <p>
            中转站和上游模型对请求内容的留存、训练使用、删除与跨境处理，仍受管理员最终选择的服务和该服务政策约束。维护者必须持续核对真实中转站的数据政策并在配置变化时更新本说明。请勿向学习助手发送密码、Cookie、Token、API

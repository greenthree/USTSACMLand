...
- [ ] 排查并优化真实成员 WebChat 请求的输入缓存命中：已确认固定探针使用默认隐式缓存而真实请求曾强制纯 `explicit` 模式；当前实现改为默认隐式模式 + 历史用户消息显式断点，并把探针升级为“长首轮 + 追加第二轮”的 typed Responses 会话形状，继续保留稳定模型/提示词缓存键和脱敏 Usage 汇总。仍不能以探针成功替代真实会话验收；退出条件为真实授权账号的连续多轮对话能够在中转站日志及脱敏服务端指标中观察到输入缓存命中，并有不增加越权、隐私泄露或错误复用风险的回归测试与生产证据。

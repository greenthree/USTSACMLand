...
- [ ] 接入大模型的 AI 聊天学习助手，用于知识问答、代码讲解和训练复盘，使用根目录下的 `WEBCHATROADMAP.md` 规划（安全 API、流式 `/assistant` 工作台、成员与全站原子配额、Vault 中转站配置、管理员逐账号授权/限额、脱敏预算告警、隐私说明和真实 Responses 协议验收均已完成。2026-07-18 生产已对齐 47 个 migration，`webchat` v5 为 ACTIVE；PR #62 新增管理员试运行观测面板，只汇总已显式配置账号的北京时间今日请求/Token、剩余额度、活动请求和最近请求时间，不读取对话正文。PostgreSQL 17 空库现执行 27 个 pgTAP 文件、679 项断言；生产只读烟测确认 7 个显式配置账号、北京时间日期与剩余额度公式一致，Pages run `29637181622` 的 build、deploy 和生产榜单审计通过，证据见 `docs/evidence/webchat-pilot-observability-production-2026-07-18.md`。当前 7 个授权账号不自动等同于真实观察完成；剩余任务是由项目负责人确认正式试运行名单并完成连续观察）。

...
- [ ] 排查并优化真实成员 WebChat 请求的输入缓存命中：固定探针原先使用默认隐式缓存而真实请求使用纯 `explicit` 模式；尝试统一为“默认隐式 + 历史显式断点”后，2026-07-19 生产追加式探针在首个上游请求收到 HTTP 400，证明当前中转站不接受该参数组合，因此已恢复已知可用的 `explicit` 模式。探针仍升级为“长首轮 + 追加第二轮”的 typed Responses 会话形状，继续保留稳定模型/提示词缓存键和脱敏 Usage 汇总。待冷却后再次运行 explicit 追加式探针，并用真实授权账号完成长首轮对话；退出条件为中转站日志及脱敏服务端指标至少一次出现 `cached_tokens > 0`，且回归测试证明不增加越权、隐私泄露或错误复用风险。

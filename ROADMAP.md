...
- [ ] 服务端校验文件签名、解码结果和像素上限，移除 EXIF 等隐私元数据，不把图片内容、签名 URL 或原始文件名写入运行日志、审计详情和错误消息；上传失败、过期对象和孤儿对象有幂等清理任务。定时清理工作流由默认关闭的仓库变量 `WEBCHAT_IMAGE_CLEANUP_ENABLED` 门控；生产空队列 service-role 手工烟测已确认零重试、零死信且 Storage 对账一致，仍需真实对象删除/恢复验收后才能考虑开启 schedule。证据见 [`docs/evidence/webchat-image-foundation-production-2026-07-25.md`](./docs/evidence/webchat-image-foundation-production-2026-07-25.md)。
...
- [ ] 每次有效绑定为邀请人增加 `1,000,000` WebChat 累计 Token 额度上限，不修改其已使用 Token，也不绕过全站预算和其他服务端限额；旧流程的生产计奖烟测已完成，邮箱确认后计奖的新流程仍待真实注册复测。

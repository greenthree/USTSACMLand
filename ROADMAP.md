...
- [ ] 完成队列最大重试、指数退避和跨平台并发限额（单平台持久重试、QOJ 禁止自动重试、原子领取、故障恢复和平台并发编排已实现，migration `202607140012` 已部署且相关 pgTAP 在空库 CI 通过；仍需生产 stale-worker 恢复、到期退避和跨平台并发限额烟测）。
...
- [ ] 同步中心：已接入活动队列进度、真实记录、全体/成员/平台范围触发、影响范围确认和失败重试；migration `202607150001` 已部署且空库 CI 通过，仍需使用真实管理员完成生产 UI 的范围触发、队列进度和失败重试烟测。
- [ ] 数据源健康页：独立页面已支持 24 小时/7 天/30 天窗口、六平台成功率、耗时、最近结构错误、凭据错误和无样本状态；生产告警与投递烟测待完成。
- [x] 审计日志：接入操作者、动作、对象、时间、脱敏变更摘要和安全 CSV 导出。
- [ ] 公告管理和 CSV 导出：公告草稿/发布/定时/归档/删除、50 条游标分页、乐观锁、审计、对话框可访问性及当前筛选成员 CSV 已完成；公告 migration 已部署且空库 CI 通过，仍需真实管理员在生产完成草稿、发布/定时、归档和删除烟测。
- [x] 高风险操作二次确认；全量同步显示已验证平台账号影响范围，对话框具有初始焦点、首尾循环、Escape 关闭和跨浏览器触发器回焦。
- [ ] 管理接口添加速率限制：成员、平台绑定、手工统计、公告 RPC 与同步 Edge Function 已接入原子固定窗口限流，migration `202607150003` 已部署且 pgTAP 通过；仍需受控生产 429/Retry-After 与前端重试提示烟测。

...
- [x] XCPC ELO 上游在 2026-08-17 将选手字段从 `teamMember/rating/contests` 调整为 `name/history`，导致生产共享缓存刷新被 `schema_changed` 阻断；仓库已完成统一归一化解析与缓存回归测试（Deno 40 项、全量 Vitest 648 项通过）。2026-08-25 先部署 `sync-member` v59，后按单独批准仅部署包含同一共享解析器的 `sync-stats` v40，并只触发一次 `xcpc_elo`：3 页共 7 项全部成功、无队列失败。最终只读对账确认缓存恢复为 v4（53 名选手），有效期至 2026-08-25 11:38 UTC，`last_error_code` 与冷却均为空；7 项 XCPC 统计全部为 `fresh`，旧 Rating 保留；活动任务为 0，QOJ 在本次窗口没有任务、运行或统计写入。证据：Actions run [32813620541](https://github.com/greenthree/USTSACMLand/actions/runs/32813620541)。
...
- [ ] 按 `docs/release-checklist.md` 完成最终检查并创建 `v1.0.0` 标签。2026-08-13 已补齐当前 `main` Schema v2 备份恢复演练，并核验 Supabase Free 计划日志保留为 1 天、GitHub Actions 默认保留为 30 天、加密备份为 14 天；Firecrawl 当前额度为 98%，但供应商未公开普通作业记录的固定自动删除期限。Auth `429` 生产演练随后已由项目负责人取消；本次未修改配置、未发送登录请求且没有产生副作用，因此该门禁仍未完成。当前仍缺 Auth `429` 风险处置、Firecrawl 固定记录保留期的书面事实、学校/集训队/ICPC 名称与图形标识授权确认，以及 2026-08-18 周二 XCPC ELO/QOJ 完整批次观察，因此暂不创建标签。证据见 [`docs/evidence/v1-release-gate-audit-2026-08-13.md`](./docs/evidence/v1-release-gate-audit-2026-08-13.md)、[`docs/evidence/auth-rate-limit-recovery-production-2026-08-04.md`](./docs/evidence/auth-rate-limit-recovery-production-2026-08-04.md) 与 [`docs/evidence/database-restore-drill-2026-08-13.md`](./docs/evidence/database-restore-drill-2026-08-13.md)。

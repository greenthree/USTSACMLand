...
- [ ] 自定义时间范围的刷题增量榜、周榜和月榜（数据库成功快照基线/末值 RPC、账号换绑隔离、失败运行排除、题数回退标记、北京时间周/月/自定义范围、累计榜切换、覆盖状态、366 天服务端上限及桌面/移动端交互已实现；29 个 pgTAP 文件/719 项断言门禁、Vitest 343/343、五浏览器增量榜交互与 axe 验证通过，待主分支 CI、生产 migration、Pages 发布和生产真实快照烟测后勾选）。

...
- [x] 配置生产 Supabase，并使数据库 migration、Edge Functions 与当前目标版本一致（2026-07-19 最新部署核对确认项目已对齐 53 个 migration、0 pending，`sync-member`、`sync-stats`、`delete-account`、`change-password`、`webchat-config`、`webchat`、`webchat-cache-probe`、`firecrawl-config` 八个函数均为 ACTIVE；需要 Deno 依赖的函数统一使用仓库 import map 部署。WebChat 服务端与数据库请求开关已对显式授权账号开放，管理员试运行观测 RPC 与生产缓存探针已部署。生产 Pages 客户端入口已启用，未登录仍跳转登录，登录且获授权账号可进入工作台。部署证据见 `docs/evidence/webchat-closed-production-deployment-2026-07-17.md`、`docs/evidence/webchat-model-visibility-production-2026-07-17.md`、`docs/evidence/webchat-pages-entry-production-2026-07-18.md`、`docs/evidence/webchat-pilot-observability-production-2026-07-18.md` 与 `docs/evidence/webchat-input-cache-production-smoke-2026-07-18.md`）。
...
- [ ] 接入大模型的 AI 聊天学习助手，用于知识问答、代码讲解和训练复盘，使用根目录下的 `WEBCHATROADMAP.md` 规划（安全 API、流式 `/assistant` 工作台、成员与全站原子配额、Vault 中转站配置、管理员逐账号授权/限额、脱敏预算告警、隐私说明、私有历史会话、真实 Responses 协议验收和固定探针输入缓存命中均已完成。2026-07-19 生产已对齐 53 个 migration，`webchat` v15、`webchat-cache-probe` v7 均为 ACTIVE；成员请求与 Token 额度为保留全部历史用量的累计总限额，全站预算仍按北京时间每日重置。管理员试运行观测面板只汇总显式配置账号的累计请求/Token、剩余总额度、活动请求和最近请求时间，不读取对话正文。PR #87 的完整门禁通过，生产追加式缓存探针 run `29665061228` 在第二轮不同请求中真实命中 1792 个输入 Token；历史会话、固定探针与追加探针证据见 `docs/evidence/webchat-history-cache-production-release-2026-07-18.md`、`docs/evidence/webchat-input-cache-production-smoke-2026-07-18.md` 和 `docs/evidence/webchat-appended-cache-relay-compatibility-2026-07-19.md`。当前 7 个授权账号不自动等同于真实观察完成，剩余任务是确认正式试运行名单、完成连续观察，并按下一条任务完成真实成员缓存命中验收）。
...
- [ ] 排查并优化真实成员 WebChat 请求的输入缓存命中：2026-07-19 两次生产追加式探针分别测试“typed 显式断点 + 默认模式”和“typed 显式断点 + `explicit` 模式”，均在首个上游请求收到 HTTP 400，证明当前中转站不接受 typed `prompt_cache_breakpoint` 请求形状。PR #87 改为普通 `role/content` 消息、移除显式缓存扩展并保留稳定模型/提示词缓存键；非流式 run `29665061228` 的第二轮输入 2,356 Token、命中 1,792 Token，但真实授权成员的 7,737 Token 长首轮与 7,765 Token 追加轮均命中 0。随后 run `29666030174` 用同一探针前缀切换到真实成员采用的 `stream: true`，两轮输入仍为 2,335/2,356 Token，但 `cache_write_tokens` 与 `cached_tokens` 全部为 0；本站已从 `response.completed` 正确读取 Usage，因此问题收敛到中转站的流式 Responses 缓存策略、透传或渠道路由。下一步保持流式请求不变，只用受控探针测试请求级 `prompt_cache_options.mode = implicit` 与 `ttl = 30m`；若仍失败，再检查中转站渠道类型、请求体/请求头透传模板与渠道粘性。退出条件仍是在中转站日志及站内脱敏服务端指标至少一次观察到真实成员 `cached_tokens > 0`，并持续证明不增加越权、隐私泄露或错误复用风险。证据见 `docs/evidence/webchat-appended-cache-relay-compatibility-2026-07-19.md` 与 `docs/evidence/webchat-real-member-cache-miss-2026-07-19.md`。

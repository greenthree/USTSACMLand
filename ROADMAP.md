...
- [ ] 接入大模型的 AI 聊天学习助手，用于知识问答、代码讲解和训练复盘，使用根目录下的 `WEBCHATROADMAP.md` 规划（安全 API、流式 `/assistant` 工作台、成员与全站原子配额、Vault 中转站配置、管理员逐账号授权/限额、脱敏预算告警、隐私说明、私有历史会话、真实 Responses 协议验收、固定探针和真实成员输入缓存命中均已完成。2026-07-19 生产已对齐 55 个 migration，`webchat` v18、`webchat-cache-probe` v10 均为 ACTIVE；成员请求与 Token 额度为保留全部历史用量的累计总限额，全站预算仍按北京时间每日重置。管理员试运行观测面板只汇总显式配置账号的累计请求/Token、剩余总额度、活动请求、最近请求时间和连续观察健康状态，不读取对话正文。AI 使用权限与正式观察名单已经解耦，现有 7 个获权账号不会自动被计入观察；管理员需独立选择 3–5 名正常账号。PR #91 的数据库安全门禁在 PostgreSQL 17 空库通过 35 个 pgTAP 文件、882 项断言；真实成员第二次同会话追加请求输入 7,451 Token、缓存命中 6,912 Token。历史会话、固定探针、追加探针与名单部署证据见 `docs/evidence/webchat-history-cache-production-release-2026-07-18.md`、`docs/evidence/webchat-input-cache-production-smoke-2026-07-18.md`、`docs/evidence/webchat-appended-cache-relay-compatibility-2026-07-19.md` 和 `docs/evidence/webchat-pilot-roster-production-deployment-2026-07-19.md`。剩余任务是由管理员选定 3–5 人，并完成 168 小时连续观察与人工复核）。

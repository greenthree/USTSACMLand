...
- [x] 配置生产 Supabase，并使数据库 migration、Edge Functions 与当前目标版本一致（2026-07-18 部署核对确认项目已对齐 47 个 migration、0 pending，`sync-member`、`sync-stats`、`delete-account`、`change-password`、`webchat-config`、`webchat` 六个函数均为 ACTIVE；WebChat 使用仓库 import map 部署，服务端与数据库请求开关已对显式授权账号开放，管理员试运行观测 RPC 已部署。生产 Pages 客户端入口已启用，未登录仍跳转登录，登录且获授权账号可进入工作台。部署证据见 `docs/evidence/webchat-closed-production-deployment-2026-07-17.md`、`docs/evidence/webchat-model-visibility-production-2026-07-17.md`、`docs/evidence/webchat-pages-entry-production-2026-07-18.md` 与 `docs/evidence/webchat-pilot-observability-production-2026-07-18.md`）。
...
- [ ] 接入大模型的 AI 聊天学习助手，用于知识问答、代码讲解和训练复盘，使用根目录下的 `WEBCHATROADMAP.md` 规划（安全 API、流式 `/assistant` 工作台、成员与全站原子配额、Vault 中转站配置、管理员逐账号授权/限额、脱敏预算告警、隐私说明和真实 Responses 协议验收均已完成。2026-07-18 生产已对齐 48 个 migration，`webchat` v8 为 ACTIVE；成员请求与 Token 额度已由北京时间每日额度迁移为保留全部历史用量的累计总限额，提额可追加余额、降额会立即生效，过期 claimed/started 请求按原结算边界跨日期处理；全站请求与 Token 预算仍按北京时间每日重置。管理员试运行观测面板只汇总显式配置账号的累计请求/Token、剩余总额度、活动请求和最近请求时间，不读取对话正文。PR #64、main CI run `29639712128`、Pages run `29639931670`、28 个 pgTAP 文件/698 项断言、140 项通用浏览器测试、48 项 WebChat 专用浏览器测试、Edge Deno 门禁和生产静态资源烟测均通过，证据见 `docs/evidence/webchat-total-member-quotas-production-2026-07-18.md`。当前 7 个授权账号不自动等同于真实观察完成，剩余任务是由项目负责人确认正式试运行名单并完成连续观察）。
- [ ] 为 AI 学习助手增加刷新恢复、历史会话与首正文前“思考中”：私有会话表、own-history RPC、每账号/会话容量限制、180 天清理、注销级联、生产正文不进入 localStorage、五浏览器刷新/切换/删除门禁均已在当前分支实现，待第 50 个 migration、Edge Function、Pages 和生产烟测发布后勾选。
- [ ] 验证并优化 WebChat 输入缓存：生产请求已使用由模型与系统提示词版本派生且不含成员或正文的稳定 `prompt_cache_key`；手动中转站工作流新增两次相同 1024 Token 以上长前缀探针，第二次必须返回 `cached_tokens > 0`。待候选分支 CI、部署和真实中转站受控探针通过后勾选。
...
- [x] 完成 AI 学习助手隐私披露：说明消息会转发给管理员配置的中转站及上游模型、当前生产版的私有额度账本最小字段边界；当前分支已同步补充私有历史会话的容量、保留、本人删除、注销级联和管理员默认不可读边界，待历史会话功能发布时一并更新线上披露。真实中转站的留存、训练、删除与跨境政策仍作为持续验收项。

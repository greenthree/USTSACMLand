...
- [ ] 排查并优化真实成员 WebChat 请求的输入缓存命中：2026-07-19 两次生产追加式探针分别测试“typed 显式断点 + 默认模式”和“typed 显式断点 + `explicit` 模式”，均在首个上游请求收到 HTTP 400，证明当前中转站不接受 typed `prompt_cache_breakpoint` 请求形状。PR #87 改为普通 `role/content` 消息、移除显式缓存扩展并保留稳定模型/提示词缓存键；生产 run `29665061228` 的第二轮追加请求输入 2356 Token，其中 `cached_tokens = 1792`，已证明默认隐式缓存能复用字节不变的早期前缀。剩余退出条件是用真实授权账号完成长首轮与追加对话，并在中转站日志及脱敏服务端指标至少一次观察到 `cached_tokens > 0`；回归测试还需持续证明不增加越权、隐私泄露或错误复用风险。证据见 `docs/evidence/webchat-appended-cache-relay-compatibility-2026-07-19.md`。

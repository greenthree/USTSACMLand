...
- [ ] 接入大模型的 AI 聊天学习助手，用于知识问答、代码讲解和训练复盘，使用根目录下的 `WEBCHATROADMAP.md` 规划（安全 API、流式 `/assistant` 工作台、成员与全站原子配额、Vault 中转站配置、管理员逐账号授权/限额、脱敏预算告警、隐私说明、私有历史会话和真实 Responses 协议验收均已完成。2026-07-18 生产已对齐 50 个 migration，`webchat` v9 为 ACTIVE；成员请求与 Token 额度为保留全部历史用量的累计总限额，全站预算仍按北京时间每日重置。管理员试运行观测面板只汇总显式配置账号的累计请求/Token、剩余总额度、活动请求和最近请求时间，不读取对话正文。PR #67、main CI run `29646206220`、Pages run `29646490889`、30 个 pgTAP 文件/738 项断言、150 项通用浏览器测试、58 项 WebChat 专用浏览器测试、Edge Deno 门禁和生产静态资源烟测均通过，证据见 `docs/evidence/webchat-history-cache-production-release-2026-07-18.md`。当前 7 个授权账号不自动等同于真实观察完成，剩余任务是由项目负责人确认正式试运行名单并完成连续观察）。
...
- [ ] 验证并优化 WebChat 输入缓存：生产请求已使用由模型与系统提示词版本派生且不含成员或正文的稳定 `prompt_cache_key`；仓库新增 service-role-only `webchat-cache-probe`、全站预算原子账本和手动 `WebChat production cache probe`，改为从 Supabase Vault 读取管理员已保存的 Base URL、模型和 Key，不再要求把三项中转站配置复制到 GitHub。探针固定发送两次字节一致的 1024 Token 以上长前缀请求，不扣成员额度、不自动重试，第二次必须返回 `cached_tokens > 0`；完成 PostgreSQL 17 CI、生产 migration/Function 部署并取得真实命中 Artifact 后勾选。

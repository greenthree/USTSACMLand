...
- 已停止：全局开关必须由私有数据库配置和仅管理员可调用的 RPC 强制执行，并包含原因、乐观锁、原子限流和审计记录；注册触发器、邀请码校验、账号页摘要和奖励事务都读取同一状态，不能只隐藏前端入口。生产关闭/重开烟测和重开安全闸门均已完成，当前共 71 个 production migration；仓库当前 48 个 pgTAP 文件、1206 项断言已在干净数据库通过。2026-07-26 新增的本地双连接验证器已真实观察配置行锁等待、全字段 CAS、响应丢失精确对账和零残留，仍待受控生产双连接烟测。证据见 [`docs/evidence/referral-concurrency-local-2026-07-26.md`](./docs/evidence/referral-concurrency-local-2026-07-26.md)。

...
- [ ] 允许管理员在后台配置、启停和轮换多个 Firecrawl API Key：Vault 多 Key Schema、后台“数据源健康”管理面板、逐 Key 一次性额度检查、零额度/过期检查启用围栏、优先级 + 最久未使用轮转、认证隔离/限流冷却、牛客延迟回退、旧环境 Key 兼容边界、QOJ operation ID 数据库一次性 claim 与旧队列零重试 fencing、实时管理员复核、原因、乐观锁、限流和脱敏审计已实现；额度告警等级由数据库按观测比例复核，密钥只进入 Supabase Vault，浏览器只看到脱敏状态。PR #83 的 PostgreSQL 17 空库门禁已通过 33 个 pgTAP 文件/845 项断言，生产已对齐 53 个 migration，`sync-member` v41、`sync-stats` v29、`firecrawl-config` v1 均为 ACTIVE；当前数据库 Key 池尚为空并继续兼容旧环境 Key。待 Pages 发布、后台录入现有 Key、检查并启用以及 QOJ/牛客真实烟测后验收。
...
- [ ] 接入大模型的 AI 聊天学习助手，用于知识问答、代码讲解和训练复盘，使用根目录下的 `WEBCHATROADMAP.md` 规划（安全 API、流式 `/assistant` 工作台、成员与全站原子配额、Vault 中转站配置、管理员逐账号授权/限额、脱敏预算告警、隐私说明、私有历史会话、真实 Responses 协议验收、固定探针和真实成员输入缓存命中均已完成。2026-07-19 生产已对齐 55 个 migration，`webchat` v18、`webchat-cache-probe` v11 均为 ACTIVE；成员请求与 Token 额度为保留全部历史用量的累计总限额，全站预算仍按北京时间每日重置。管理员试运行观测面板只汇总显式配置账号的累计请求/Token、剩余总额度、活动请求、最近请求时间和连续观察健康状态，不读取对话正文。AI 使用权限与正式观察名单已经解耦，现有 7 个获权账号不会自动被计入观察；管理员需独立选择 3–5 名正常账号。PR #91 的数据库安全门禁在 PostgreSQL 17 空库通过 35 个 pgTAP 文件、882 项断言；真实成员第二次同会话追加请求输入 7,451 Token、缓存命中 6,912 Token。历史会话、固定探针、追加探针与名单部署证据见 `docs/evidence/webchat-history-cache-production-release-2026-07-18.md`、`docs/evidence/webchat-input-cache-production-smoke-2026-07-18.md`、`docs/evidence/webchat-appended-cache-relay-compatibility-2026-07-19.md` 和 `docs/evidence/webchat-pilot-roster-production-deployment-2026-07-19.md`。剩余任务是由管理员选定 3–5 人，并完成 168 小时连续观察与人工复核）。
...
- [x] 排查并优化真实成员 WebChat 请求的输入缓存命中：typed 内容块级 `prompt_cache_breakpoint` 在默认与 `explicit` 模式均收到 HTTP 400，因此历史保持普通 `role/content` 消息。非流式 run `29665061228` 的追加轮命中 1,792 Token，流式默认策略 run `29666030174` 未命中；单变量 run `29666900027` 显式声明请求级 `mode=implicit` 与 `ttl=30m` 后追加轮命中 1,792 Token。随后生产键探针 run [`29667755656`](https://github.com/greenthree/USTSACMLand/actions/runs/29667755656) 同样命中 1,792 Token，排除了该键持续粘性到不兼容渠道的假设。`webchat` v18 只为直接 GPT-5.6+ 模型 ID 发送已验证策略，并将其计入额度指纹和保守预留；旧模型与自定义模型保持原请求形状。真实授权成员第一次新策略请求输入 7,423 Token、命中 0，第二次同会话追加请求输入 7,451 Token、命中 6,912 Token，站内私有账本与中转站 Usage 一致，且无自动重试、越权、正文日志或错误复用。证据见 `docs/evidence/webchat-appended-cache-relay-compatibility-2026-07-19.md` 与 `docs/evidence/webchat-real-member-cache-miss-2026-07-19.md`。
- [ ] 复核中转站后台“当前无法缓存命中输入”与站内/Usage 曾返回 `cached_tokens` 的观测差异：PR #92 已增加客户端/上游/Responses 请求 ID、实际模型、请求与共享前缀指纹、缓存写入/读取遥测和 streaming / non-streaming 单变量对照，生产 `webchat-cache-probe` 已部署为 ACTIVE v11。run `29665061228` 曾在追加轮命中 1,792 Token；随后流式 run `29672691083` 与非流式 run `29673642726` 均为 `cache_write_tokens=0`、`cached_tokens=0`，后者实际模型仍为 `gpt-5.6-sol` 且诊断为 `no_cache_write_or_read`。输入长度、前缀复用、缓存键、请求级 `implicit/30m`、Usage 解析和传输方式均已排除，当前最可能是中转站具体渠道不支持 GPT-5.6 缓存或没有转发缓存字段。下一步需使用脱敏请求 ID 在中转站后台确认真实渠道、上游模型和字段透传，并固定到支持缓存的渠道；修复后再做一次无自动重试生产探针，命中前保持未完成。证据见 `docs/evidence/webchat-cache-route-diagnostics-2026-07-19.md`。
...
- [x] 增加 AI 学习助手正式试运行连续观察健康摘要：管理员只读 RPC 要求 3–5 人，并连续观察 168 小时；后台只显示成员覆盖、成功/不完整/失败请求、未知 Usage、活动生成和缓存命中等聚合指标，不返回身份、请求编号、对话正文或密钥。第 54 个 migration 已部署，PR #90 的完整 CI、浏览器、Edge 与 861 项数据库断言通过；名单选择和真实 168 小时观察仍由 WebChat 总功能条目跟踪。
- [x] 将 AI 助手使用权限与正式试运行观察名单解耦：第 55 个 migration 新增私有名单字段和独立观察时钟，既有获权账号默认不纳入；管理员可从正常获权账号中独立选择最多 5 人，名单、正式成员权限/额度或账号状态变化都会重新开始观察。事务 advisory lock 防止并发加入第 6 人，停用成员仍占用名额以避免恢复后形成 6 人名单，兼容旧权限 RPC 且不撤销任何现有 AI 权限。PR #91 的 `verify`、`database-security` 和 `gitleaks` 全部通过，PostgreSQL 17 空库完成 35 个 pgTAP 文件、882 项断言；生产已部署并核对 0 pending，证据见 `docs/evidence/webchat-pilot-roster-production-deployment-2026-07-19.md`。

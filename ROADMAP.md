...
- [x] 配置生产 Supabase，并使数据库 migration、Edge Functions 与当前目标版本一致（2026-07-19 最新部署核对确认项目已对齐 54 个 migration、0 pending，`sync-member`、`sync-stats`、`delete-account`、`change-password`、`webchat-config`、`webchat`、`webchat-cache-probe`、`firecrawl-config` 八个函数均为 ACTIVE；需要 Deno 依赖的函数统一使用仓库 import map 部署。WebChat 服务端与数据库请求开关已对显式授权账号开放，管理员试运行观测 RPC、连续观察健康摘要 RPC 与生产缓存探针已部署。生产 Pages 客户端入口已启用，未登录仍跳转登录，登录且获授权账号可进入工作台。部署证据见 `docs/evidence/webchat-closed-production-deployment-2026-07-17.md`、`docs/evidence/webchat-model-visibility-production-2026-07-17.md`、`docs/evidence/webchat-pages-entry-production-2026-07-18.md`、`docs/evidence/webchat-pilot-observability-production-2026-07-18.md` 与 `docs/evidence/webchat-input-cache-production-smoke-2026-07-18.md`）。
...
- [ ] 接入大模型的 AI 聊天学习助手，用于知识问答、代码讲解和训练复盘，使用根目录下的 `WEBCHATROADMAP.md` 规划（安全 API、流式 `/assistant` 工作台、成员与全站原子配额、Vault 中转站配置、管理员逐账号授权/限额、脱敏预算告警、隐私说明、私有历史会话、真实 Responses 协议验收、固定探针和真实成员输入缓存命中均已完成。2026-07-19 生产已对齐 54 个 migration，`webchat` v18、`webchat-cache-probe` v10 均为 ACTIVE；成员请求与 Token 额度为保留全部历史用量的累计总限额，全站预算仍按北京时间每日重置。管理员试运行观测面板只汇总显式配置账号的累计请求/Token、剩余总额度、活动请求、最近请求时间和连续观察健康状态，不读取对话正文。PR #90 的数据库安全门禁在 PostgreSQL 17 空库通过 34 个 pgTAP 文件、861 项断言；真实成员第二次同会话追加请求输入 7,451 Token、缓存命中 6,912 Token。历史会话、固定探针与追加探针证据见 `docs/evidence/webchat-history-cache-production-release-2026-07-18.md`、`docs/evidence/webchat-input-cache-production-smoke-2026-07-18.md` 和 `docs/evidence/webchat-appended-cache-relay-compatibility-2026-07-19.md`。当前 7 个授权账号不自动等同于真实观察完成，剩余任务是将正式试运行名单收敛为 3–5 人，并完成 168 小时连续观察与人工复核）。
...
- [ ] 复核中转站后台“当前无法缓存命中输入”与站内/Usage 曾返回 `cached_tokens` 的观测差异：使用同一会话连续追加请求采集脱敏元数据，逐项检查实际模型路由、稳定 `prompt_cache_key`、缓存模式与 TTL、系统提示词版本、工具定义及历史消息序列化是否发生前缀漂移；区分后台展示口径、具体渠道不支持和客户端请求不稳定三类原因，修复后补充生产实测证据与防回归测试。
...
- [x] 增加 AI 学习助手正式试运行连续观察健康摘要：管理员只读 RPC 以当前显式授权且账号正常的成员组成正式名单，要求 3–5 人，并从最近一次已启用成员权限/额度变更起连续观察 168 小时；后台只显示成员覆盖、成功/不完整/失败请求、未知 Usage、活动生成和缓存命中等聚合指标，不返回身份、请求编号、对话正文或密钥。第 54 个 migration 已部署，PR #90 的完整 CI、浏览器、Edge 与 861 项数据库断言通过；名单调整和真实 168 小时观察仍由 WebChat 总功能条目跟踪。

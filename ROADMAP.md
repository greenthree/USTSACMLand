...
- [x] 配置生产 Supabase，并使数据库 migration、Edge Functions 与当前目标版本一致（2026-07-17 部署核对确认项目 `ACTIVE_HEALTHY`、43 个 migration、0 pending、schema lint 0，`sync-member`、`sync-stats`、`delete-account`、`change-password`、`webchat-config`、`webchat` 六个函数均为 ACTIVE；WebChat 两个函数使用仓库 import map 首次部署，`CHAT_ENABLED=false`，localhost CORS 预检为 200，匿名请求进入正常 401 鉴权边界。关闭态部署证据见 `docs/evidence/webchat-closed-production-deployment-2026-07-17.md`）。
...
- [ ] 完成生产 RLS 审计、管理员账号保护和最小权限检查（严格就绪检查已确认项目健康、43 个 migration、0 pending、六个函数、schema lint 0、八个私有资源拒绝匿名访问、五个公开视图不含 QQ/角色/审核状态，以及正式/恶意 Origin 和匿名方法边界；PR #57 的 PostgreSQL 17 空库 CI 已通过 22 个 pgTAP 文件、551 项权限/事务断言。2026-07-16 生产随机普通、停用和管理员三身份真实 JWT 矩阵 11/11 通过，证据见 `docs/evidence/production-rls-identity-matrix-2026-07-16.md`；随后两个随机成员完成真实管理员提升、接管、降级和两条审计，已有 JWT 随数据库角色即时获得/失去后台权限，目标绑定租约删除后两个旧 JWT 均无法读取、写入私有 Profile 或调用后台，Auth/Profile 零残留，证据见 `docs/evidence/production-admin-role-handoff-old-jwt-2026-07-16.md`。此前烟测也已确认资料触发器、错误密码拒绝、双会话改密与全局失效、注销 `503` 失败关闭；仍需配置恢复令牌后的成功自助注销、Storage/受控约束 `409`、双连接锁与响应丢失对账验收）。
...
- [ ] 每日一题、题目讨论和完成记录。
- [ ] 接入大模型的 AI 聊天学习助手，用于知识问答、代码讲解和训练复盘，使用根目录下的 `WEBCHATROADMAP.md` 规划（安全 API、成员与全站原子日配额、环境/数据库双重后端关闭和默认隐藏的 `/assistant` 前端工作台已完成；前端支持流式输出、停止、重新生成、复制、清空、Markdown/代码块和结构化错误恢复，聊天依赖保持独立懒加载。管理员后台 `/admin/webchat` 已支持修改中转站 Base URL、固定模型、API Key、成员请求开关与全站北京时间每日请求/Token 预算；Key 仅写入 Supabase Vault，配置读取与审计均脱敏。成员详情新增默认拒绝的逐人 AI 助手授权，以及管理员可设置的北京时间每日请求/Token 上限；成员 `/assistant` 只显示自己的当日已用、预留、剩余额度和重置时间。WebChat 在读取 Vault 配置前先检查成员授权，数据库原子 claim 还会二次读取账号状态、成员授权、逐人额度、数据库总开关与全站预算，撤权、暂停和降额不会被预读竞态绕过；旧的无授权 claim/start 已从 `service_role` 撤权。后台同时展示当日全站请求、已结算/预留 Token、剩余额度和北京时间重置时间；请求/Token 预算首次阻断会在数据库原子标记后各投递一次不含成员、请求、消息、地址或 Key 的脱敏 Webhook，避免告警风暴且投递故障不改变额度拒绝结果。本地五浏览器矩阵已覆盖键盘停止、减少动画、移动端 axe、10 个独立浏览器会话不串流和传输层 10 路同时流。已新增只允许手动运行的真实中转站验收器与 GitHub 工作流，覆盖非流式、Responses typed SSE、Usage、首增量后 Abort 和脱敏 14 天证据。PR #57 的 `verify`、`database-security`、`gitleaks` 已全部通过，其中 PostgreSQL 17 空库执行 22 个 pgTAP 文件、551 项断言；2026-07-17 已将四个 WebChat migration 和 `webchat-config`/`webchat` v1 部署到生产，43 个 migration 无 pending，两个函数均为 ACTIVE，`CHAT_ENABLED=false` 且 localhost CORS 预检为 200，证据见 `docs/evidence/webchat-closed-production-deployment-2026-07-17.md`。当前仍未配置真实中转站，生产 `VITE_WEBCHAT_UI_ENABLED=false`、`CHAT_ENABLED=false` 与数据库请求开关继续关闭；还需完成真实中转站兼容性验收、后台生产配置和 3–5 人受控启用）。
- [ ] 自定义时间范围的刷题增量榜、周榜和月榜。

...
- [ ] 验证并优化 WebChat 输入缓存：生产请求已使用由模型与系统提示词版本派生且不含成员或正文的稳定 `prompt_cache_key`；手动中转站工作流新增两次相同 1024 Token 以上长前缀探针，第二次必须返回 `cached_tokens > 0`。首次生产触发 run `29646649195` 在请求前因 GitHub Actions 尚未配置 `CHAT_RELAY_BASE_URL`、`CHAT_RELAY_API_KEY`、`CHAT_RELAY_MODEL` 安全失败，未产生模型费用；补齐三个仓库 Secret 并取得真实命中证据后勾选。

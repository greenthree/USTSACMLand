...
- [ ] 排查并优化真实成员 WebChat 请求的输入缓存命中：2026-07-19 typed 内容块级 `prompt_cache_breakpoint` 在默认与 `explicit` 模式均收到 HTTP 400，因此历史保持普通 `role/content` 消息。非流式 run `29665061228` 的追加轮命中 1,792 Token，但真实授权成员的 7,737/7,765 Token 两轮均命中 0；流式对照 run `29666030174` 同样写入 0、命中 0。单变量 run `29666900027` 只增加请求级 `prompt_cache_options.mode=implicit` 与 `ttl=30m`，追加轮输入 2,356 Token、命中 1,792 Token；但相同策略临时用于既有生产缓存键时，真实成员第一条追加请求以 `upstream_http_error` 终止且没有可信 Usage，说明独立探针键与生产键很可能被当前 New API 系中转站粘性到能力不同的渠道。实验性成员改动已撤回，生产恢复原可用形状。下一步让受控探针使用生产缓存键验证同一渠道；若仍错误，则需在中转站统一渠道能力、开启请求体透传/正确的 `codex cli trace` 粘性，或更换稳定支持 GPT-5.6 流式缓存的渠道。退出条件仍是在中转站日志及站内脱敏服务端指标至少一次观察到真实成员 `cached_tokens > 0`，并持续证明不增加越权、隐私泄露或错误复用风险。证据见 `docs/evidence/webchat-appended-cache-relay-compatibility-2026-07-19.md` 与 `docs/evidence/webchat-real-member-cache-miss-2026-07-19.md`。

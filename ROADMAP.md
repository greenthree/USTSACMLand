...
- [ ] 服务端校验文件签名、解码结果和像素上限，移除 EXIF 等隐私元数据，不把图片内容、签名 URL 或原始文件名写入运行日志、审计详情和错误消息；上传失败、过期对象和孤儿对象有幂等清理任务。定时清理工作流由默认关闭的仓库变量 `WEBCHAT_IMAGE_CLEANUP_ENABLED` 门控，图片 migration 与函数部署完成前不会定时请求生产端点，受控手工执行仍保留。

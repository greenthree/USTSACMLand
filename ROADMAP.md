...
- [ ] 排查并优化真实成员 WebChat 请求的输入缓存命中：2026-07-19 两次生产追加式探针分别测试“typed 显式断点 + 默认模式”和“typed 显式断点 + `explicit` 模式”，均在首个上游请求收到 HTTP 400，证明当前中转站不接受 typed `prompt_cache_breakpoint` 请求形状。PR #87 改为普通 `role/content` 消息、移除显式缓存扩展并保留稳定模型/提示词缓存键；生产 run `29665061228` 的第二轮追加请求输入 2356 Token，其中 `cached_tokens = 1792`，已证明非流式探针能用默认隐式缓存复用字节不变的早期前缀。真实授权成员随后完成 7,737 Token 长首轮与同会话追加第二轮，第二轮输入 7,765 Token，但两次 `cached_tokens` 均为 0，与中转站日志一致。下一轮优先用同一探针请求体隔离 `stream: false` 与 `stream: true`，再逐项检查 `safety_identifier`、真实系统提示词和流式 SSE Usage 映射。退出条件仍是在中转站日志及站内脱敏服务端指标至少一次观察到真实成员 `cached_tokens > 0`，并持续证明不增加越权、隐私泄露或错误复用风险。证据见 `docs/evidence/webchat-appended-cache-relay-compatibility-2026-07-19.md` 与 `docs/evidence/webchat-real-member-cache-miss-2026-07-19.md`。

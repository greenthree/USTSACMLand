...
- [ ] 接入大模型的 AI 聊天学习助手，用于知识问答、代码讲解和训练复盘，使用根目录下的 `WEBCHATROADMAP.md` 规划（安全 API、流式 `/assistant` 工作台、成员与全站原子配额、Vault 中转站配置、管理员逐账号授权/限额、脱敏预算告警、隐私说明、私有历史会话、真实 Responses 协议验收和固定探针输入缓存命中均已完成。2026-07-19 生产已对齐 53 个 migration，`webchat` v10、`webchat-cache-probe` v2 均为 ACTIVE；成员请求与 Token 额度为保留全部历史用量的累计总限额，全站预算仍按北京时间每日重置。管理员试运行观测面板只汇总显式配置账号的累计请求/Token、剩余总额度、活动请求和最近请求时间，不读取对话正文。PR #69 的 PostgreSQL 17 门禁通过 31 个 pgTAP 文件/779 项断言，生产缓存探针 run `29650242439` 的第二次相同长前缀请求真实命中 1792 个输入 Token；历史会话发布及缓存证据见 `docs/evidence/webchat-history-cache-production-release-2026-07-18.md` 与 `docs/evidence/webchat-input-cache-production-smoke-2026-07-18.md`。当前 7 个授权账号不自动等同于真实观察完成，剩余任务是确认正式试运行名单、完成连续观察，并按下一条任务解决真实成员请求未命中输入缓存的问题）。
...
- [ ] 排查并优化真实成员 WebChat 请求的输入缓存命中：2026-07-19 两次生产追加式探针分别测试“typed 显式断点 + 默认模式”和“typed 显式断点 + `explicit` 模式”，均在首个上游请求收到 HTTP 400；这证明当前中转站不接受 typed `prompt_cache_breakpoint` 请求形状，而不是单纯缺少请求级 mode。当前实现已恢复为中转站此前验证可成功请求的普通 `role/content` 消息，不发送显式缓存扩展，继续保留稳定模型/提示词缓存键和“长首轮 + 追加第二轮”真实会话探针，由默认隐式缓存复用字节不变的早期前缀。待 30 分钟冷却后只运行一次新版探针，并用真实授权账号完成长首轮对话；退出条件为中转站日志及脱敏服务端指标至少一次出现 `cached_tokens > 0`，且回归测试证明不增加越权、隐私泄露或错误复用风险。失败证据与参数边界见 `docs/evidence/webchat-appended-cache-relay-compatibility-2026-07-19.md`。

...
- [ ] 每日一题、题目讨论和完成记录。
- [ ] 接入大模型的 AI 聊天学习助手，用于知识问答、代码讲解和训练复盘，使用根目录下的 `WEBCHATROADMAP.md` 规划（安全 API、原子配额与默认隐藏的 `/assistant` 前端工作台已完成；前端支持流式输出、停止、重新生成、复制、清空、Markdown/代码块和结构化错误恢复，聊天依赖保持独立懒加载。生产 `VITE_WEBCHAT_UI_ENABLED=false` 与 `CHAT_ENABLED=false` 继续关闭；还需完成真实中转站 Responses SSE/Usage/Abort 兼容性验收、生产 migration/Function 部署、Secrets 配置和受控启用）。
- [ ] 自定义时间范围的刷题增量榜、周榜和月榜。

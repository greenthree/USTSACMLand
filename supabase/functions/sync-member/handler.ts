...
import { shouldNotifySyncFailure, type SyncFailureAlert } from '../_shared/alerts.ts'
import { runtimeErrorAlert, type RuntimeErrorAlert } from '../_shared/error-monitoring.ts'
...
export interface SyncMemberPlatformAccount {
  id: number
  profile_id: string
  platform: PlatformId
  external_id: string
...
export interface SyncMemberAdapterContext {
  client: SupabaseClient
  account: SyncMemberPlatformAccount
  jobId: number
  attempt: number
  runId: number
}

export interface SyncMemberHandlerDependencies {
  allowedOrigins?: string
  createClient(): { client: SupabaseClient; serviceRoleKey: string }
  resolveAdapter(
    context: SyncMemberAdapterContext,
  ): PlatformAdapter | null | Promise<PlatformAdapter | null>
  now(): Date
  notifySyncFailure(alert: SyncFailureAlert): Promise<void>
  notifyRuntimeError(alert: RuntimeErrorAlert): Promise<void>
}
...
async function persistResult(
  ...
  triggerType: SyncTriggerType,
  dependencies: SyncMemberHandlerDependencies,
  ...
  try {
    ...
    const adapter = await dependencies.resolveAdapter({
      client,
      account,
      jobId,
      attempt,
    ...
    if (!adapter) {
      throw new Error(`${account.platform} adapter is unavailable`)
    }
...
export function createSyncMemberHandler(
  dependencies: SyncMemberHandlerDependencies,
  ...
  return async (request) => {
    ...
    try {
      ...
      serviceClient = createdClient.client
      const serviceRoleKey = createdClient.serviceRoleKey
      ...
      if (
        memberRegistrationSync &&
        !isRegistrationSyncWindowOpen(auth.profileCreatedAt, dependencies.now().getTime())
      ) {
      ...
      const accounts = ((accountRows ?? []) as SyncMemberPlatformAccount[]).filter((account) =>
      ...
      if (jobId === null) {
        ...
        const { data: job, error: jobError } = await serviceClient
          ...
          .insert({
            ...
            scheduled_for: new Date(dependencies.now().getTime() + 60_000).toISOString(),
          })
      ...
      if (completion.status === 'failed' && alertFailures.length > 0) {
        await dependencies.notifySyncFailure({
          jobId,
          triggerType,
          attempt: jobAttempt,
          maxAttempts: jobMaxAttempts,
          failedAt: completion.transitionedAt ?? dependencies.now().toISOString(),
      ...
      if (!(error instanceof ApiError) && !(error instanceof AdminRateLimitError)) {
        await dependencies.notifyRuntimeError(
          runtimeErrorAlert('sync-member', request, error, dependencies.now),
        )
      }

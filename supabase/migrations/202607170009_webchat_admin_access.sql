-- Administrators are also training-team accounts. Permit approved member and
-- administrator profiles to receive the same deny-by-default WebChat policy,
-- while preserving suspension, quota, audit, and paid-request boundaries.
...
begin
  ...
  -- Keep the same deterministic administrator-row order as role handoff. The
  -- caller cannot be demoted or suspended before this policy change commits.
  perform 1
...
create or replace function public.claim_authorized_webchat_request(
...
create or replace function public.mark_authorized_webchat_request_started(
  requested_user_id uuid,
  requested_request_id text,
  requested_owner_token uuid
)
...
comment on function public.admin_get_webchat_member_access(uuid) is
  'Returns one member or administrator WebChat policy, using disabled version-zero defaults when no private row exists.';
comment on function public.admin_update_webchat_member_access(
  uuid, boolean, integer, bigint, bigint, text
) is 'Changes bounded member or administrator WebChat access with live administrator checks, rate limiting, optimistic locking, and redacted audit.';
comment on function public.read_webchat_member_runtime_access(uuid) is
  'Returns service-only member or administrator eligibility and private WebChat quota policy without profile details.';
comment on function public.read_own_webchat_usage() is
  'Returns only the JWT account current Beijing-day WebChat usage, effective access, limits, and remaining quota.';
comment on function public.claim_authorized_webchat_request(
  uuid, text, text, uuid, integer, bigint, integer
) is 'Atomically rechecks account, private access, relay switch, account quotas, and global quotas before claiming paid WebChat work.';
comment on function public.mark_authorized_webchat_request_started(uuid, text, uuid) is
  'Starts a claimed WebChat request only while the relay, account, and private authorization remain enabled.';

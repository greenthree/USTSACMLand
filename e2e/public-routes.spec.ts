...
test('shared referral links do not activate invitation inputs while referral program is closed', async ({
  ...
  await expect(page.getByRole('heading', { name: '创建账号' })).toBeVisible({ timeout: 20_000 })
  await expect(page.getByRole('textbox', { name: '邀请码（选填）' })).toHaveCount(0)
  await expect
    .poll(() =>
      page.evaluate(
...
test('demo member account page does not show referral program while closed', async ({ page }) => {
  await page.addInitScript(() => {
    window.sessionStorage.setItem('usts-acm-land-demo-session:v1', 'member@example.edu.cn')
  })
  await page.goto('/account')

  await expect(page.getByRole('heading', { name: '我的资料' })).toBeVisible({ timeout: 20_000 })
  await expect(page.getByRole('heading', { name: '推荐计划' })).toHaveCount(0)
  await expect(page.getByRole('button', { name: '复制注册链接' })).toHaveCount(0)
})

...
for (const width of [390, 1440, 1920]) {
  test(`campus contests show school then practice with individual rules at ${width}px`, async ({
    ...
    await page.goto('/')
    // WebKit cannot reliably scroll a descendant inside an offscreen
    // content-visibility:auto section, so bring the section into layout first.
    await page.locator('.home-join-section').scrollIntoViewIfNeeded()
    ...
    const schoolContestLink = page.getByRole('link', { name: '了解校赛' })
    await expect(schoolContestLink).toHaveAttribute('href', '/contests/campus')
    await schoolContestLink.scrollIntoViewIfNeeded()
    await expect(schoolContestLink).toBeInViewport()
    await schoolContestLink.click()
